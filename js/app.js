...
(function (global) {
  ...
  // WebP変換・ZIP書き出し・クリップボードは imageWorkflow.js に切り出し済み。
  // v2（作り直し中の情報設計）からも同じロジックを呼べるようにするため。
  const ImageWorkflow = global.ImageWorkflow;
  ...
  function setSlotImage(slot, dataURL) {
    imageStore[slot.id] = { file: ImageWorkflow.slotFileName(slot.id, dataURL), dataURL: dataURL };
    syncImagesToState();
    ...
    slots.forEach(function (slot) {
      ...
      input.addEventListener('change', function () {
        ...
        reader.onload = function () {
          ImageWorkflow.toWebpDataURL(f, reader.result, function (finalDataURL) {
  ...
  /** ZIP書き出し本体（WebP変換・CRC32・ZIP組み立て）は imageWorkflow.js 側。
   *  ここでは「画像が無い」ときのトースト表示だけ担当する。 */
  function exportImagesZip() {
    const ok = ImageWorkflow.downloadImagesZip(imageStore, 'lp-images.zip');
    toast(ok ? '画像を書き出しました（展開して index.html と同じ場所へ）' : '先に画像をセットしてね');
  }
  ...
  /** クリップボード書き込み本体（file:// 向けの execCommand フォールバック込み）は
   *  imageWorkflow.js 側。ここでは呼び出し名だけ既存のまま維持する薄いラッパ。 */
  function writeClipboard(text, done) {
    ImageWorkflow.writeClipboard(text, done);
  }
