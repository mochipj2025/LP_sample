...
(function (global) {
  ...
  function imageSlots(state) {
    ...
    order.forEach(function (id) {
      ...
      for (let k = 0; k < def.count; k++) {
        const sid = def.count > 1 ? id + '-' + (k + 1) : id;
        let label = '';
        let subject = '';
        ...
        if (id === 'fv') {
          ...
          const item = (k === 0 && state.infoProduct)
            ? '「' + state.infoProduct + '」'
            : (k === 0 ? '一番おすすめの' + (imageType ? imageType.menu : '商品・メニュー')
              : (k === 1 ? '二番目の' + (imageType ? imageType.menu : '商品・メニュー')
                : '三番目の' + (imageType ? imageType.menu : '商品・メニュー')));
          label = menuLabel(g) + 'の写真｜' + (k === 0 && state.infoProduct ? state.infoProduct : (k === 0 ? '1品目・推し' : (k + 1) + '品目'));
          subject = imageType
            ? item + (k === 0 && state.infoProduct ? 'を主役にした写真。' : 'の写真。') + MENU_ANGLE[k % 3]
  ...
  const FIELDS = [
    ...
    { key: 'infoProduct', label: '商品・メニュー', icon: '🍽️', group: 'info', type: 'textarea', rows: 1, random: false, placeholder: '例）初回体験コース' },
    { key: 'infoPrice', label: '価格', icon: '💴', group: 'info', type: 'textarea', rows: 1, random: false, placeholder: '例）初回 3,980円' },
  ...
  const DEFAULT_STATE = {
    ...
    volume: '標準',
    output: '構成の設計図',
    infoName: '', infoProduct: '', infoPrice: '', infoDate: '', infoPlace: '', infoFeature: '',
    ref: '', extra: ''
  };
